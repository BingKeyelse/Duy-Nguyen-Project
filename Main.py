...
class MainWindow(QMainWindow):
    ...
    def on_page_changed(self): # gắn giá trị để không phải lúc nào cũng lấy ảnh từ trigger phần cứng 
    ...
    def trigger_cam_1(self): # Lẩy ảnh với Cam 1 để xử lý
        self.mode_pic_cam1.value=0
        """Gọi trigger để chụp ảnh"""
        self.camera.trigger_cam_special()  

    def trigger_cam_sample(self): # Lấy ảnh với Cam 1 để truyền vào chương trình Sample để Temple matching
        self.mode_pic_cam1.value=2
        """Gọi trigger để chụp ảnh"""
        self.camera.trigger_cam_special()

    def trigger_cam_calib(self): # Lấy ảnh với Cam 1 để truyền vào chương trình Calib
        if self.file_count_calib <7: # Khi khởi tạo Calib thì biết này vì nó cập nhập theo Timer, lấy liên tục
            self.mode_pic_cam1.value=1 
            """Gọi trigger để chụp ảnh"""
            self.camera.trigger_cam_special()
        
    def trigger_4cam(self): # Chụp ảnh với 4 Cam của AI
        self.camera.trigger_4cam()

    def update_process_4cam_with_slider(self): # Hàm thay đổi giá trị 4 Cam AI khi thay đổi QSlider
    ...
    def process_queue_4cam_cam2(self):# Hàm dành riêng cho cam 2 nhưng cũng là phần xử lý tín hiệu luôn khi tổng hợp giá trị của 4 Cam
        while True:
            time.sleep(0.001)
            try:
    ...
    def process_queue_4cam_cam3(self):
        while True:
            time.sleep(0.001)
            if self.running==True:
                try:
    ...
    def process_queue_4cam_cam4(self):
        while True:
            time.sleep(0.001)
            if self.running==True:
                try:
    ...
    def process_queue_4cam_cam5(self):
        while True:
            time.sleep(0.001)
            if self.running==True:
                try:
    ...
    def process_queue_cam1(self): # Process xử lý cho Cam 1
        while True:
            time.sleep(0.00001)
            if self.running==True:
    ...
    def auto_calib(self): # Hàm tự động chạy Calib
    ...
    def update_end_day_production(self):
    ...
    def update_sum_ok_ng_production(self):
        db_path = os.path.join(self.current_file_path, "data_txt", self.database_production[0])
    ...
    def insert_new_data_into_database_production(self): # Hàm ở phần Sản lượng khi thêm một giá trị mới muốn theo dõi
        self.value_const_production_new= Cam_1.check_value_line_edit_production(self)
        if self.value_const_production_new != self.value_const_production:
            self.value_const_production_new = self.value_const_production
            self.ui.lineEdit.setText(str(self.value_const_production))
    ...
    def read_database_production(self):
        db_path = os.path.join(self.current_file_path, "data_txt", self.database_production[0])
        conn = sqlite3.connect(db_path)
        cursor = conn.cursor()
    ...
    def save_value_production(self):
        self.value_const_production= self.value_const_production_new
        self.update_value_production()

    def update_value_production(self):
        # Đường dẫn file
    ...
    def init_production(self):# Hàm khởi động khi chạy chương trình tính Sản lượng
    ...
    def monitor_month_change(self): # Giám sát thay đổi giờ tháng năm theo thời gian sau 1 minute
...
class camera_Basler_multi:
    ...
    def trigger_4cam(self):
        for i, cam in enumerate(self.cam_group):
            time.sleep(0.002)
            self.trigger_cam(cam, self.group_queues[i])
            
    def trigger_cam_special(self):
